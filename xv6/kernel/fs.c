...
#define ChecksumMask 0xff000000
#define AddressMask  0x00ffffff
...
stati(struct inode *ip, struct stat *st) {
  ...
  if(ip->type == T_CHECKED) 
  {
    for(loop = 0; loop < (NDIRECT + 1); loop++) 
    {
      if(loop == NDIRECT) 
      {
        bp = bread(ip->dev, ip->addrs[NDIRECT]);
        ptr = (uint *)bp->data;
        for(loop2 = 0; loop2 < NINDIRECT; loop2++)
          checksum ^= (uchar) ((ptr[loop2] & ChecksumMask) >> 24);
         brelse(bp);
       } 
       else {
        checksum ^= (uchar) ( ( ip->addrs[loop] & ChecksumMask ) >> 24);
      }
    }
...
uint
computeChecksum(uchar *data) {
  int loop;
  uint checksum;
  uchar byte_checksum = 0;
  for( loop = 0; loop < BSIZE; loop++)
    byte_checksum ^= data[loop];
  checksum = (uint)byte_checksum;
...
uint 
getChecksum(struct inode *ip, uint b_num)
{
  struct buf *bp;
  uint checksum;
  uint *ptr;
  if(b_num < NDIRECT) {
    checksum = ip->addrs[b_num] & ChecksumMask;
    return checksum;
  }
  b_num -= NDIRECT;
  if(b_num < NINDIRECT) {
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    ptr = (uint *)bp->data;
    checksum = ptr[b_num] & ChecksumMask;
    brelse(bp);
    return checksum;
  }
  panic("getChecksum ");
...
int
updateChecksum(struct inode *ip, uint block_num, uint checksum) {
  struct buf *bp;
  uint *ptr;
  if(block_num < NDIRECT) {
    ip->addrs[block_num] &= 0x00ffffff;
    ip->addrs[block_num] |= checksum;
    return 0;
  }
  block_num -= NDIRECT;
  if(block_num < NINDIRECT) {
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    ptr = (uint *)bp->data;
    ptr[block_num] &= 0x00ffffff;
    ptr[block_num] |= checksum;
    bwrite(bp);
    brelse(bp);
    return 0;
  }
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    if(ip->type == T_CHECKED) {
      if(getChecksum(ip, off/BSIZE) != computeChecksum(bp->data)) {
        brelse(bp);
        return -1;
      }
    }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    bwrite(bp);
    if(ip->type == T_CHECKED)
      updateChecksum(ip, off/BSIZE, computeChecksum(bp->data));
    brelse(bp);
  }
